    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants.length
          ? details.participants.map((participant) => `
              <span class="participant-chip" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}">
      ...
      activitiesList.querySelectorAll('.delete-participant').forEach((btn) => {
        btn.addEventListener('click', async (e) => {
          ...
          try {
            const response = await fetch(`/activities/${activity}/signup?email=${email}`, { method: 'DELETE' });
            if (response.ok) {
              fetchActivities();
            } else {
